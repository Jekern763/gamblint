...
def mock_dynamodb(mocker):
    with mock_aws():
        ...
        dynamodb = boto3.resource('dynamodb', region_name='us-east-1', aws_access_key_id="dummy", aws_secret_access_key="dummy")
...
def test_create_session(mock_dynamodb, gateway):
    ...
    response = mock_dynamodb.get_item(Key={"session_id": SESSION_ID, "record_type": "mid_session"})
...
def test_get_active_session(mock_dynamodb, gateway):
    ...
    response = mock_dynamodb.get_item(Key={"session_id": SESSION_ID, "record_type": RecordType.MID_SESSION})
...
def test_commit_guess_transaction(mock_dynamodb, gateway):
    ...
    response = mock_dynamodb.get_item(Key={"session_id": SESSION_ID, "record_type": str(RecordType.SESSION_LOG)})
